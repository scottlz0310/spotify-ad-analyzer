...
各レビューコメントは以下のいずれかに分類する：

- **承認（ACCEPTED）** — 完全に対応する
- **一部承認（PARTIALLY ACCEPTED）** — 一部のみ対応し、理由を説明する
- **却下（REJECTED）** — 対応しない理由を説明する

### 実装ルール（承認 / 一部承認の場合）

1. 関連するコードを修正する
2. リポジトリの規約に従っていることを確認する
3. 変更は最小限かつ局所的に留める
4. 動作が変更される場合はテストを更新する

### コミット形式

```
fix: Copilotのレビューコメントに対応 <概要>
```

例:
```
fix: Copilotのレビューコメント（Null安全性）への対応
fix: Copilotのレビューコメント（エラー処理）への対応
```
...
課題が完全に処理された場合、コメントに返信してから会話を解決（resolve）する。

### 安全規則

- 関係のないコードをリファクタリングしない
- 必要がないのにパブリック API を変更しない
- 指示がない限りテストを削除しない
- 互換性を破る変更を導入しない
